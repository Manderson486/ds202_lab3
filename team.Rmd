...
```
```{r}
#data %>% group_by(Marital.status) %>% summarise(ed = mean(Education)) %>% ggplot(aes(x=reorder(Marital.status, ed), weight=ed))+geom_bar(show.legend = FALSE) + xlab(aes(fill = 'Marital.status')"Marital Status") + ylab("Average Number of Years Educated")
```
